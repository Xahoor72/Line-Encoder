...
var x_offset = x_axis + 0.5; 
var y_offset = y_axis + 0.5; 
let isBinary=true;
function longest_consec_zeroes(text){
    let N=text.length;
    let c=0;
    let mx=0;
    let start =-1,end=-1;
...
function checkString( bit_code) {
    
    for (let i = 0; i < bit_code.length; i++) {
      if (bit_code[i] == '0' || bit_code[i] == '1') {
  ...
  function validate_bit_code() {
    checkString(bit_code);
    if (!(isBinary)) {
        alert("Enter only zeroes and ones !");
        ...
        x_steep = x_scale;
        //lps(bit_code);
      // longest_consec_zeroes(bit_code)
...
function draw() {
    var canvasN = document.getElementsByClassName("axis");
